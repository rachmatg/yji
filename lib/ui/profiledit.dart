...
class ProfilePageState extends State<ProfilePage> {
  ...
  //PageController merokokController = new PageController();
...
//  final FocusNode myFocusNodeClubName = FocusNode();
  ...
  void initState()
  {
    getUserProfile().then((d){
      ...
      _listPenyakit = <String>[penyakit1];
      List<UserProfile> users;
      userCollection.where("mobile",isEqualTo:userNumber).snapshots().listen((data){
        final List<UserProfile> users = data.documents.
        map((documentSnapshot) => UserProfile.fromMap(documentSnapshot.data))
            .toList();
        //setState(() {
          this.userProfiles = users;
          _listPenyakit = json.decode(this.userProfiles[0].penyakit);
          signupNameController.text= this.userProfiles[0].name;
          signupEmailController.text = this.userProfiles[0].email;
          signupMobileController.text = this.userProfiles[0].mobile;
//          print (this.userProfiles.toString());
        //});
      });
    });
  }
            ...
            child: ListView(
              ...
              children: <Widget>[
                ...
                TextFormField(
                  ...
                  onSaved: (val)=>userProfile.mobile = val,
                  validator: (value) {
                    if (value.isEmpty) {
...
//                      print(userProfile.kotaClub);
  ...
  Future<void> _addUser(UserProfile user) async {
    MmkvFlutter mmkv = await MmkvFlutter.getInstance();
    await mmkv.setString('phone',signupMobileController.text);
    ...
    db.createUser(
      ...
      jenisClub: user.jenisClub,
      namaClub: signupClubNameController.text,
...
*/
  void _submitForm() {
    ...
    print('Phone: ${userProfile.mobile}');
    print('========================================');
